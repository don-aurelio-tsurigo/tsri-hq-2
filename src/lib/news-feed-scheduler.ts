...
export function startNewsFeedScheduler() {
  ...
  const tick = async () => {
    ...
    try {
      ...
      console.log(
        `[news-feed] scheduled fetch: orgs=${summary.orgs} inserted=${summary.inserted} fetched=${summary.fetched} missing=${summary.missing} enriched=${summary.enriched} purged=${summary.purged}`,
      );
    } catch (err) {
