...
import { toggleProjectNavPin } from "@/lib/actions";
...
export function ProjectPinButton({
  ...
  return (
    <span className="relative inline-flex">
      <button
        ...
        onClick={(e) => {
          ...
          startTransition(async () => {
            ...
            fd.set("id", projectId);
            const result = await toggleProjectNavPin(fd);
            if (result?.error) {
      ...
      {error ? (
        <span className="absolute top-full left-0 z-10 mt-1 w-max max-w-[12rem] rounded-md bg-white px-2 py-1 text-xs text-red-700 shadow">
          {error}
        </span>
      ) : null}
    </span>
